...
/***********************************I-DEP-MAY-ADQ-0-16/10/2020****************************************/
CREATE OR REPLACE VIEW adq.vsolicitud_compra_v2(
    id_solicitud,
    id_proveedor,
    desc_proveedor,
    id_moneda,
...
AS
  SELECT DISTINCT sol.id_solicitud,
   ...
   <TH>Cantidad</TH>
   <TH>P / U
       ('::text || mon.codigo::text) || ')</TH>'::text) || pxp.html_rows((((((((
         ('<td>'::text || COALESCE(ci.desc_ingas::text, '-'::text)) || '</td>
       <td>'::text) || COALESCE(sd.descripcion, '-'::text)) || '</td>
           <td>'::text) || COALESCE(sd.cantidad::text, '-'::text)) ||
             '</td> <td>'::text) || COALESCE(round(sd.precio_unitario, 2)::text,
             '-'::text)) || '</td> '::text)::character varying)::text) ||
  ...
  GROUP BY sol.id_solicitud,
           ...
           dep.nombre,
           sol.id_funcionario_rpc;
/***********************************F-DEP-MAY-ADQ-0-16/10/2020****************************************/

/***********************************I-DEP-MAY-ADQ-1-16/10/2020****************************************/
CREATE OR REPLACE VIEW adq.vcotizacion_v2(
    id_cotizacion,
    numero_oc,
    codigo_proceso,
    num_cotizacion,
...
AS
  ...
  GROUP BY cot.id_cotizacion,
           ...
           sol.precontrato,
           fun.fecha_asignacion
  ORDER BY fun.fecha_asignacion;
/***********************************F-DEP-MAY-ADQ-1-16/10/2020****************************************/

/***********************************I-DEP-MAY-ADQ-0-30/10/2020****************************************/
DROP VIEW adq.vsolicitud_compra_v2;

CREATE VIEW adq.vsolicitud_compra_v2 (
    id_solicitud,
    id_proveedor,
    desc_proveedor,
    id_moneda,
...
SELECT DISTINCT sol.id_solicitud,
   ...
   <TH>P / U
       ...
       <td>'::text) || COALESCE(sd.descripcion, '-'::text)) || '</td>
           <td>'::text) || COALESCE(sd.cantidad::text, '-'::text)) || '</td>
               <td>'::text) || COALESCE(round(sd.precio_unitario, 2)::text, '-'::text)) || '</td> '::text)::character varying)::text) || '</table>'::text AS detalle,
    fun.desc_funcionario1,
    uo.codigo AS codigo_uo,
    uo.nombre_unidad,
    lower(sol.tipo::text) AS tipo,
...
ALTER VIEW adq.vsolicitud_compra_v2
  OWNER TO "postgres";


DROP VIEW adq.vcotizacion_v2;

CREATE VIEW adq.vcotizacion_v2 (
    id_cotizacion,
    numero_oc,
    codigo_proceso,
    num_cotizacion,
...
GROUP BY cot.id_cotizacion, cot.numero_oc, pro.codigo_proceso,
    pro.num_cotizacion, sol.id_solicitud, p.id_proveedor, p.desc_proveedor, sol.id_moneda, sol.id_depto, sol.numero, sol.fecha_soli, sol.estado, sol.num_tramite, sol.id_gestion, sol.justificacion, p.rotulo_comercial, sol.id_categoria_compra, cac.codigo, cac.nombre, mon.codigo, fun.desc_funcionario1, uo.codigo, uo.nombre_unidad, sol.tipo_concepto, sol.tipo, fun.nombre_cargo, fun.nombre_unidad, cot.requiere_contrato, cot.nro_contrato, sol.precontrato, fun.fecha_asignacion, sol.tipo_modalidad, sol.proceso_contratacion
ORDER BY fun.fecha_asignacion;

ALTER VIEW adq.vcotizacion_v2
  OWNER TO "postgres";



/***********************************F-DEP-MAY-ADQ-0-30/10/2020****************************************/

/***********************************I-DEP-MAY-ADQ-0-20/1/2021****************************************/
CREATE OR REPLACE VIEW adq.vsolicitud_compra_rep(
    id_solicitud,
    id_proveedor,
    desc_proveedor,
    id_moneda,
...
AS
  ...
  FROM adq.tsolicitud sol
       ...
       JOIN param.tmoneda mon ON mon.id_moneda = sol.id_moneda
       JOIN param.tdepto dep ON dep.id_depto = sol.id_depto
       JOIN orga.vfuncionario_cargo_v2 fun ON fun.id_funcionario =
         sol.id_funcionario AND fun.estado_reg_asi::text = 'activo'::text AND
         fun.tipo_funcionario::text = 'oficial'::text
       JOIN orga.tuo uo ON uo.id_uo = sol.id_uo
       JOIN segu.vusuario usu ON usu.id_usuario = sol.id_usuario_reg
       LEFT JOIN param.vproveedor p ON p.id_proveedor = sol.id_proveedor
       LEFT JOIN adq.tcategoria_compra cac ON cac.id_categoria_compra =
...
FROM adq.tsolicitud sol
     ...
     LEFT JOIN adq.tcategoria_compra cac ON cac.id_categoria_compra =
         sol.id_categoria_compra
WHERE fun.fecha_asignacion <= sol.fecha_soli AND (fun.fecha_finalizacion >=
    sol.fecha_soli OR fun.fecha_asignacion <= sol.fecha_soli) AND (fun.fecha_asignacion <= sol.fecha_soli OR fun.fecha_finalizacion IS NULL)
GROUP BY sol.id_solicitud, sol.id_proveedor, p.desc_proveedor, sol.id_moneda,
    sol.id_depto, sol.numero, sol.fecha_soli, sol.estado, sol.num_tramite, sol.id_gestion, sol.justificacion, p.rotulo_comercial, sol.id_categoria_compra, cac.codigo, cac.nombre, mon.codigo, fun.desc_funcionario1, uo.codigo, uo.nombre_unidad, sol.tipo_concepto, sol.tipo, fun.nombre_cargo, fun.nombre_unidad, fun.email_empresa, usu.desc_persona, dep.prioridad, dep.nombre, sol.id_funcionario_rpc;

ALTER VIEW adq.vsolicitud_compra
  OWNER TO postgres;
/***********************************F-DEP-MAY-ADQ-0-20/1/2021****************************************/