...
?>
<script>
    Phx.vista.VerificarPresupuesto = Ext.extend(Phx.gridInterfaz, {

        constructor: function(config) {
            this.maestro = config;
            Phx.vista.VerificarPresupuesto.superclass.constructor.call(this, config);
            this.init();
            this.addButton('inserAuto',{ 
                text: 'Revertir ', 
                iconCls: 'blist', 
                disabled: false, 
                handler: this.revertirParcial, 
        ...
        ActList : '../../sis_adquisiciones/control/Solicitud/listarSolicitudCEP',
                    ...
                    if(confirm("Realmente seguro?, no podrá retroceder")){
                        ...
                        for(var i=0;i<filas.length;i++){
                            if(filas[i].data.revertir != 0){
                                //armar array de ids
                                id_sol_dets[i] = filas[i].data.id_solicitud_det;
                        ...
                        if(sw){
                            console.log('me.maestro.id_solicitud: ',me.maestro.id_solicitud, 'id_sol_dets: ',id_sol_dets.toString(), 'revertir: ',rev.toString());
                            Ext.Ajax.request({
                                url:'../../sis_adquisiciones/control/Solicitud/revertirParcialmentePresupuesto',
                                params: {
                                    id_sol_dets: id_sol_dets.toString(),
                                    id_solicitud: me.maestro.id_solicitud,
