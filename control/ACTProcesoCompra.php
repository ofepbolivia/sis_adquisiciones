...
require_once(dirname(__FILE__) . '/../reportes/RepProcContraDet.php');
require_once(dirname(__FILE__) . '/../reportes/RepProcAdj.php');
...
class ACTProcesoCompra extends ACTbase
{
    ...
    function listarProcesoCompra()
    {
        ...
        if (strtolower($this->objParam->getParametro('estado')) == 'pendientes') {
            $this->objParam->addFiltro("(pc.estado in (''pendiente'',''proceso'')) and (array_estados_cot is NULL  or  ''borrador''=ANY(array_estados_cot))");
        }

        if (strtolower($this->objParam->getParametro('estado')) == 'anulados') {
            $this->objParam->addFiltro("(pc.estado in (''pendiente'',''proceso'')) and (''anulado''=ANY(array_estados_cot))");
        }


        if (strtolower($this->objParam->getParametro('estado')) == 'iniciados') {
            $this->objParam->addFiltro("(pc.estado in (''pendiente'',''proceso'')) and (''cotizado''=ANY(array_estados_cot)  or ''adjudicado''=ANY(array_estados_cot) or ''recomendado''=ANY(array_estados_cot))");
        }
        if (strtolower($this->objParam->getParametro('estado')) == 'contrato') {
            $this->objParam->addFiltro("(pc.estado in (''pendiente'',''proceso'')) and ( ''contrato_pendiente''=ANY(array_estados_cot) or ''contrato_elaborado''=ANY(array_estados_cot))");
        }

        if (strtolower($this->objParam->getParametro('estado')) == 'en pago') {
            $this->objParam->addFiltro(" (''pago_habilitado''=ANY(array_estados_cot)  or  ''finalizada''=ANY(array_estados_cot))  and pc.estado != ''finalizado'' ");
        }
        if (strtolower($this->objParam->getParametro('estado')) == 'finalizados') {
            $this->objParam->addFiltro("(pc.estado in (''finalizado'',''anulado''))");
    ...
    function reporteProcesosContraDet()
    {
        ...
        $this->objParam->addParametro('tipo', 'iniciados');
        $this->objFunc = $this->create('MODProcesoCompra');
        $this->res = $this->objFunc->reporteProcesosContraDet($this->objParam);
        $this->objParam->addParametro('iniciados', $this->res->datos);
        ...
        $this->objParam->addParametro('tipo', 'contrato');
        $this->objFunc = $this->create('MODProcesoCompra');
        $this->res = $this->objFunc->reporteProcesosContraDet($this->objParam);
        $this->objParam->addParametro('contrato', $this->res->datos);

        $this->objParam->addParametro('tipo', 'compraservicio');
        $this->objFunc = $this->create('MODProcesoCompra');
        $this->res = $this->objFunc->reporteProcesosContraDet($this->objParam);
        $this->objParam->addParametro('compraservicio', $this->res->datos);
        ...
        $this->objReporteFormato = new RepProcContraDet($this->objParam);
        $this->objReporteFormato->imprimeIniciados();
        $this->objReporteFormato->imprimeAdjudicados();
        $this->objReporteFormato->imprimeConContrato();
        $this->objReporteFormato->imprimeConOrdenComSer();
        $this->objReporteFormato->imprimeEjecutados();
        $this->objReporteFormato->imprimeConcluidos();
    ...
    function reporteProcesosContraAdj()
    {
        ...
        $this->objFunc = $this->create('MODProcesoCompra');
        $this->res = $this->objFunc->reporteProcesosContraAdj($this->objParam);
        //var_dump( $this->res);exit;
        //obtener titulo del reporte
        $titulo = 'RepProcAdj';
        ...
        $this->objParam->addParametro('datos', $this->res->datos);
        //Instancia la clase de excel
        $this->objReporteFormato = new RepProcAdj($this->objParam);
